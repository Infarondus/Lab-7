...
def get_weather(city):
    ...
    try:
        ...
        data = resp.json()
        return {'city': data['name'], 'temp': data['main']['temp'], 'feels_like': data['main']['feels_like'],
            'description': data['weather'][0]['description'], 'humidity': data['main']['humidity'], 'pressure': data['main']['pressure'], 'wind_speed': data['wind']['speed']}
    except Exception as e:
        return {'error': str(e)}

def get_steam_news(appid=440, count=5):
    url = 'http://api.steampowered.com/ISteamNews/GetNewsForApp/v0002/'
    params = {'key': steam_key, 'appid': appid, 'count': count, 'format': 'json'}
    try:
        resp = requests.get(url, params=params, timeout=10)
        resp.raise_for_status()
        data = resp.json()
        ...
        for item in news_items:
            result.append({'title': item['title'],'author': item.get('author', 'Unknown'),
